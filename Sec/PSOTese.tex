...
possui conhecimento sobre como seus vizinhos se comportam. Essas duas informações correspondem à aprendizagem individual (cognitiva) e à transmissão cultural (social), respectivamente. Portanto, cada decisão estará associado a seu desempenho passado e de alguns de seus vizinhos \cite{Serapiao2009}.
...
Em que $\phi_1$ e $\phi_2$ são números aleatórios positivos com distribuição uniforme, geralmente denominados respectivamente como componentes ``cognitivo'' e ``social''.
...
\section{PSO Matricial Simplificado}
		...
		Apesar de muitas evidências empíricas acumuladas mostrarem que o PSO é uma excelente ferramenta de otimização, até então, pouco se pesquisou sobre o seu funcionamento e convergência \cite{Clerc2002}. Com base nessa motivação o algoritmo para a atualização das partículas, velocidade, melhor local e melhor global, é reorganizado em uma forma que a análise de convergência seja possível. A forma encontrada para a reescrita do algoritmo foi colocando-o na forma um sistema dinâmico discreto.
		
		Para que seja possível organizar o PSO na forma matricial algumas definiçõoes são necessárias. Primeiramente é necessário reescrever a condição de atualização das melhores partículas locais conforme as equações \ref{eq:Condicao2} e \ref{eq:Condicao3}. 
		
		\begin{equation}
		\left\{\begin{matrix}
		Se ( J_{PSO}(X^i(k)) < J_{PSO}(P^i_B(k)) ) \Rightarrow Verdadeiro = 1 \\ \\
...
\end{equation}
		...
		\begin{equation}
		\left\{\begin{matrix}
		Se ( J_{PSO}(X^i(k)) < J_{PSO}(P^i_B(k)) ) \Rightarrow Falso = 0\\ \\
		1-s_1^i(k) = \frac{sign (J_{PSO}(X^i(k)) - J_{PSO}(P^i_B(k)))+1}{2}\\ \\
		\end{matrix}\right.
	\label{eq:Condicao3}
\end{equation}
		
		Sendo que $J_{PSO}(L)$ é a função custo, que é uma função de $L$, e que são as melhores partículas locais, ou as melhores experiências locais das partículas. A função $sign$, ou sinal, é representada de acordo com a equação \ref{eq:sinal}.
	
	\begin{equation}
			\begin{array}{ll}
					sign(x) = \left\{\begin{matrix} 1, If \ x\geqslant 0 
\\-1, If \ x< 0 \end{matrix}\right.
			\end{array}
			\label{eq:sinal}
			\end{equation}
			
			Isso significa que podemos escrever as condições \ref{eq:Condicao2} e \ref{eq:Condicao3} na forma da equação \ref{eq:Mlocal}.
			
			\begin{equation}
			\begin{array}{ll}
				P^i_B(k+1) = X^i(k)s_1^i(k) + P^i_B(k)(1-s_1^i(k))
			\end{array}
			\label{eq:Mlocal}
			\end{equation}
		...
		%se ( J_{PSO}(X^i(k)) < J_{PSO}(P^i_B(k)) ) \Rightarrow Verdadeiro = 1; & P^i_B(k+1) = X^i(k) \\
		%se ( J_{PSO}(X^i(k)) < J_{PSO}(P^i_B(k)) ) \Rightarrow Falso = 0; & P^i_B(k+1) = P^i_B(k) \\
	%\end{array}\right.
	%\label{eq:Condicao1}
%\end{equation}

%Onde $J_{PSO}(L)$ é a função custo, que é uma função de L, ou seja, das partículas, melhores locais ou global.
...
%\end{equation}
		%
		%Onde a função sinal é representada conforme a equação \ref{eq:sinal}.
	%
	%\begin{equation}
			%\begin{array}{ll}
					%sign(x)= s_1^i(k) = s_2(k) = \left\{\begin{matrix} 1, se \ x\geqslant 0 
%\\-1, se \ x< 0 \end{matrix}\right.
			%\end{array}
			%\label{eq:sinal}
			%\end{equation}
			%
			%Isso significa que é possível escrever as condiçõoes \ref{eq:Condicao2} e \ref{eq:Condicao3} na forma da equação \ref{eq:Mlocal}.
			%
			%\begin{equation}
			%\begin{array}{ll}
				%P^i_B(k+1) = X^i(k)s_1^i(k) + P^i_B(k)(1-s_1^i(k))
		...
		Tendo reescrito a condição de atualização das melhores partículas locais, a segunda condição para construir a variação matricial do PSO é reescrever a condição de atualização da melhor partícula global. Para isso, foi buscada uma solução, em princípio baseada na simplificação desenvolvida por ~\cite{Mendes2004}. Na simplificação realizada pelos autores utiliza-se um conceito de média entre as melhores partículas locais e globais com o intuito de melhorar o algoritmo. Percebendo a possibilidade de utilização de tal conceito para a sintetização de uma relação semelhante a da equação \ref{eq:Mlocal}, foi definido que a atualização da partícula global seria realizada a partir da média das partículas locais, conforme a equação \ref{eq:Pmedia}.
			...
			Com isso, é possível a partir da condição \ref{eq:CondicaoPg} obter a equação \ref{eq:MGlobal}:
			
			\begin{equation}
			\left\{\begin{array}{llcc}
				se ( J_{PSO}(P_m(k)) < J_{PSO}(P_G(k)) ) \Rightarrow Verdadeiro = 1; & P_G(k+1) = P_m(k) \\
				se ( J_{PSO}(P_m(k)) < J_{PSO}(P_G(k)) ) \Rightarrow Falso = 0; & P_G(k+1) = P_G(k) \\
			\end{array}\right.
			\label{eq:CondicaoPg}
			\end{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		\begin{equation}
			...
			\begin{equation}
			G_{D}^i(k)= \frac{[s(E_{P_G}^i(k)) + s(ME_{P_G}^i(k))] + [s(E_{P_G}^i(k))\,s(ME_{P_G}^i(k))] + 1}{4}
			\label{eq:GD}
			\end{equation}
		...
		Em que $s_{3}(x)$ é a função sinal, definida como:
...
%\end{equation}
	...
	Suponhamos que a equação \ref{eq:aT} possa ser reescrita na forma da equação \ref{eq:aT2}.
		...
		A partir do conceito de autovalores é possível provar que a estabilidade assintótica do ponto fixo do sistema linear depende do m\'odulo dos autovalores de ``A'', matriz da equação \ref{eq:aT2}. Isto é, o ponto de equilíbrio é assintóticamente estável quando o m\'odulo de todos os seus autovalores é menor que 1, para um sistema discreto. Em consequ\^encia, quando, pelo menos, o m\'odulo de um autovalor é maior que 1 o sistema é instável \cite{Monteiro2002}. Isto significa que:
...
\end{itemize}
		...
		Para aplicar os conceitos apresentados é necessário supor que o sistema é linear e invariante no tempo. Dessa maneira, serão assumidas tais hipóteses de maneira a analisar parte do algoritmo. 
...
Como o comportamento de $s_1^i(k)$ e $s_2(k)$ é equivalente a uma chave, ou seja, assume valores ``0'' ou ``1'', as possibilidades formam o conjunto que calcula as novas partículas, o valor das melhores locais e a melhor partícula global para o algoritmo de PSO matricial. Com isso, é possível reorganizar o sistema da seguinte maneira:
...
\textit{Teorema 1}: O sistema \ref{eq:aT3} converge para zero quase certamente se e somente se todos os autovalores da matriz $\frac{\sum^{n}_{i=1}{A_i}}{n} $ se encontram no interior do ciclo de raio unitário.

Baseado no teorema apresentado a matriz da equação \ref{eq:matMAs}, substituídas com as 4 possibilidades, resultariam numa nova matriz $\bar{A}$, que pode ser visualizada na equação \ref{eq:matMAbs} cujos autovalores determinam a estabilidade quase certa das partículas.
...
Com base na matriz da equação \ref{eq:matMAbs2} é possível realizar uma simulação em busca de regiões estáveis, que são as possíveis escolhas de parâmetros que resultam em uma estabilidade quase certa. A figura \ref{fig:EstabilidadeQuaseCerta} apresenta uma região formada com os parâmetros no $R^4$ ($\phi_1$, $\phi_2$, $\omega$ e $\lambda_{esq}$), porém, apresentada em pares, onde os parâmetros que não são apresentados em cada gráfico foram fixados iguais a 1.
...
\centering
   %\FONTE{\cite{franca}}
   \includegraphics[scale=0.485]{Figuras/EstabilidadeQuaseCerta.png}
      \caption{Gráficos com regiões de estabilidade no $R^4$ com os parâmetros não apresentados iguais a 1}
   \label{fig:EstabilidadeQuaseCerta}
\end{figure}


Portanto, é possível determinar uma região de estabilidade quase certa para o sistema linear por partes, apresentado na equação \ref{eq:aT3}. Isso significa que a comutação entre as matrizes que geram esse sistema quase certamente resultará na convergência do estado T(k), e, em consequência, na convergência das partículas ($X$), das velocidades ($V$), das melhores locais ($P_B$) e das melhores globais ($P_G$).
...
\end{equation}	
		...
		Para que seja possível perceber quais são as condições necessárias para que o erro diminua a inequação pode ser expandida. Conforme as inequações \ref{eq:DeducaoErro1}, \ref{eq:DeducaoErro2}, \ref{eq:DeducaoErro3}, \ref{eq:DeducaoErro4}, \ref{eq:DeducaoErro5}, \ref{eq:DeducaoErro6}, \ref{eq:DeducaoErro7}:
	  ...
	  \end{equation}
		...
		\begin{equation}
			\begin{matrix}
				- 2\,y\,S\,A(k)\,A(k-1)\,T(k-1) + (S\,A(k)\,A(k-1)\,T(k-1))^2 +2\,y\,S\,A(k-1)\,T(k-1) - \\(S\,A(k-1)\,T(k-1))^2 < 0
			\end{matrix}
		\label{eq:DeducaoErro2}
	  \end{equation}
		
		\begin{equation}
			\begin{matrix}
				- 2\,y\,S\,A(k)\,A(k-1)\,T(k-1) + (S\,A(k)\,A(k-1)\,T(k-1)\,S\,A(k)\,A(k-1)\,T(k-1)) +\\2\,y\,S\,A(k-1)\,T(k-1) - (S\,A(k-1)\,T(k-1)\,S\,A(k-1)\,T(k-1)) < 0
			\end{matrix}
		\label{eq:DeducaoErro3}
	  \end{equation}
		
		\begin{equation}
			\begin{matrix}
				(2\,y\,S\,I - 2\,y\,S\,A(k))\,A(k-1)\,T(k-1) + \\(S\,A(k)\,A(k-1)\,T(k-1)\,S\,A(k)\,A(k-1)\,T(k-1)) - \\(S \, A(k-1) \, T(k-1) \, S \, A(k-1)\,T(k-1)) < 0
			\end{matrix}
		\label{eq:DeducaoErro4}
	  \end{equation}
		
		\begin{equation}
			\begin{matrix}
				2\,y\,S\,(I-A(k))\,A(k-1)\,T(k-1) + (S\,A(k)\,A(k-1)\,T(k-1)\,S\,A(k)\,A(k-1)\,T(k-1)) - \\(S\,A(k-1)\,T(k-1)\,S\,A(k-1)\,T(k-1)) < 0
			\end{matrix}
		\label{eq:DeducaoErro5}
	  \end{equation}
		
		\begin{equation}
			\begin{matrix}
				2\,y\,S\,(I-A(k))\,A(k-1)\,T(k-1) + ((S\,A(k)\,A(k-1)\,T(k-1)\,S\,A(k)) - \\(S\,A(k-1)\,T(k-1)\,S))\,A(k-1)\,T(k-1) < 0
			\end{matrix}
		\label{eq:DeducaoErro6}
	  \end{equation}
	  ...
	  \end{equation}
		...
		Ao analisar a inequação \ref{eq:DeducaoErro7} conjuntamente com a forma como as matrizes $A(k)$ e $A(k-1)$ são montadas (tendo como base a equação 	\ref{eq:matA}), é possível perceber que existem $4*4$ possibilidades, $4$ para $A(k)$ e $4$ para $A(k-1)$. Dessa maneira temos:
		...
		Como pode ser percebido, existem apenas $3$ possibilidades de ocorrer a diminuição do erro em direção ao zero, descritos pelas condições expressas nas inequações \ref{eq:ConvErro1}, \ref{eq:ConvErro2} e \ref{eq:ConvErro3}.
			...
			Algo que é possível perceber com isso é que a partir da forma como as partículas se movimentam é possível estimar quando as partículas estão abaixo do ponto de equilíbrio ou acima dele.
...
%
		...
		\subsection{PSOM - Híbrido}			
			...
			Após a PSO ser reescrito na forma matricial e realizada uma análise de convergências das partículas e da otimização, foi pensado em uma maneira de combinar a PSO com algoritmos de otimização determinísticos, quando a PSO é aplicada a processos com incertezas. O objetivo desta combinação é eliminar a possibilidade de uma inicialização ruim, que não permita a convergência das partículas para uma solução aceitável na PSO. Além disso, intenciona-se obter desempenhos melhores que os algoritmos tradicionais. Ou seja, associando algoritmos determinísticos com o PSO espera-se obter resultados tão bons ou melhores que os algoritmos deterministicos, eliminando a possibilidade de não encontrar um bom indivíduo devido à aleatoriedade do PSO.
